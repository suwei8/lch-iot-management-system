...
const AdminOrders: React.FC = () => {
  ...
  const handleRefund = (order: Order) => {
    ...
    refundForm.setFieldsValue({
      refundAmount: order.amount / 100 // 转换为元
    });
  ...
  const handleRefundSubmit = async () => {
    ...
    try {
      ...
      const response = await orderService.refundOrder(selectedOrder.id, {
        refundAmount: values.refundAmount * 100, // 转换为分
  ...
  const columns: ColumnsType<Order> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div>
            <ShopOutlined /> {record.merchant?.name || '未知商户'}
          </div>
          <div style={{ fontSize: '12px', color: '#666' }}>
            {record.store?.name || '未知门店'}
          </div>
        </div>
      )
    },
  ...
  useEffect(() => {
    fetchOrders();
    fetchStats();
  }, [currentPage, pageSize, searchText, selectedStatus, selectedMerchant, selectedStore, dateRange]);
    ...
    <div style={{ padding: '24px' }}>
        ...
        {selectedOrder && (
          <Descriptions column={1} bordered>
            ...
            <Descriptions.Item label="商户信息">
              <div>
                <div>商户名称：{selectedOrder.merchant?.name || '未知商户'}</div>
                <div>商户编码：{selectedOrder.merchant?.code || '未知编码'}</div>
              </div>
            </Descriptions.Item>
      ...
      >
        <Form form={refundForm} layout="vertical">
          ...
          >
            <Input type="number" step="0.01" placeholder="请输入退款金额" />
          </Form.Item>
